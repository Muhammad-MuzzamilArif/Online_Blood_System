...
function set_rights($menus, $menuRights, $topmenu) {
    ...
    for ($i = 0, $c = count($menus); $i < $c; $i++) {
        ...
        for ($j = 0, $c2 = count($menuRights); $j < $c2; $j++) {
            if ($menuRights[$j]["modulecode"] == $menus[$i]["mod_modulecode"]) {
                if (authorize($menuRights[$j]["rr_create"]) || authorize($menuRights[$j]["edit"]) ||
                        authorize($menuRights[$j]["rr_delete"]) || authorize($menuRights[$j]["view"])
                ) {
                    ...
                    $data[$menus[$i]["mod_modulegroupcode"]][$menuRights[$j]["modulecode"]] = $row;
